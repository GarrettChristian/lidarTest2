...
def signReplace(sign, intensitySign, semanticsSign, instancesSign, scene, intensity, semantics, instances, details):
    ...
    meshMax[2] = meshMin[2]
    meshLen = np.linalg.norm(maxSign - minSign)

    if (signHeight < -1):
        print("Sign too low min {}".format(signHeight))
...
def scaleVehicle(asset, intensityAsset, semanticsAsset, instancesAsset, 
    ...
    # instancesIncluded = instances[sceneMask]
    ...
    # instancesNotIncluded = instances[sceneMaskNot]

    # Prepare the mesh for ray casting to move points to mesh
    assetData = (asset, intensityAsset, semanticsAsset, instancesAsset)
    sceneIncludedData = (scene, intensity, semantics, instances)
    success, newAssetData, newSceneData = pointsToMesh(mesh, assetData, sceneIncludedData)    

    newAsset, newIntensityAsset, newSemanticsAsset, newinstancesAsset = newAssetData
    newScene, newIntensity, newSemantics, newinstances = newSceneData

    # print(np.shape(newAsset))
    if (success and np.shape(newAsset)[0] < 20):
        success = False

    # return success, sceneNotIncluded, intensityNotIncluded, semanticsNotIncluded, instancesNotIncluded, newAsset, newIntensityAsset, newSemanticsAsset, newinstancesAsset, details
    return success, newScene, newIntensity, newSemantics, newinstances, newAsset, newIntensityAsset, newSemanticsAsset, newinstancesAsset, details
