...
for instance in labelInstance:
    if (instance not in seenInstance and instance != 0):
        # print(instance)
        seenInstance.add(instance)

        maskInstance = (labelInstance == instance)
        curItem = pcd_arr[maskInstance, :]

        pcdItem = o3d.geometry.PointCloud()
        pcdItem.points = o3d.utility.Vector3dVector(curItem)

        obb = pcdItem.get_oriented_bounding_box()
        obb.color = (0, 1, 0)

        # o3d.visualization.draw_geometries([pcdItem, obb])
...
boxes.append(pcdCar)
boxes.append(pcdNoCar)
# o3d.visualization.draw_geometries([pcdCar, pcdNoCar])
