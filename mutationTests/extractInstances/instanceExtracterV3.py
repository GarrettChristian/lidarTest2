...
def parseSequence(folderNum, path, savePath, mdbColAssetMetadata, mdbColAssets, mutex):
    ...
    mdbColAssetMetadata.insert_one(sequenceData)
...
def threadParseSeq(threadNum, path, savePath, mdbColAssetMetadata, mdbColAssets, mutex):
    global sequences
    global globalData

    # Get folder to work on
    seq = 0
    mutex.acquire()
    if (len(sequences) > 0):
        seq = sequences.pop()
        print("Thread {} starting on seq {}".format(threadNum, seq))
    else:
        seq = None
        print("Thread {} found no seq".format(threadNum))
    mutex.release()

    # While you have a folder to work on
    while (seq != None):
        
        # Parse that folder
        folderNum = str(seq).rjust(2, '0')
        sequenceData = parseSequence(folderNum, path, savePath, mdbColAssetMetadata, mdbColAssets, mutex)

        # Update global data and start on next folder (if there is one)
        mutex.acquire()
        print("Updating global data for seq {}".format(folderNum))
...
def main():
    ...
    mdb = mongoConnect()
    mdbColAssets = mdb["assets3"]
    mdbColAssetMetadata = mdb["asset_metadata3"]
    print("Connected")

    path = "/home/garrett/Documents/data/dataset3/sequences/"
    savePath = "/home/garrett/Documents/data/dataset4/sequences/"
    os.makedirs(savePath, exist_ok=True)

    print("Parsing {} :".format(path))

    sequences = [*range(6, 11)]
    print(sequences)
    threads = 1
    ...
    for i in range(threads):
        thread = threading.Thread(target=threadParseSeq, args=(i, path, savePath, mdbColAssetMetadata, mdbColAssets, mutex))
