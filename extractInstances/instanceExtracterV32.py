...
def parseAssets(labelsFileName, binFileName, sequence, savePath, sequenceData, assetsToSave):
    ...
    print("Worker starting on {}".format(labelsFileName), flush=True)
...
def parseSequence(seq, path, savePath):

    print("Worker starting on seq {}".format(seq), flush=True)
    ...
    for index in range(len(labelFiles)):
        ...
        if (len(assetsToSave) >= 2000):
            # Save the asset data
            print("Save batch of 2000 seq {}".format(folderNum), flush=True)
...
def main():
    ...
    contextMultiprocess = multiprocessing.get_context('forkserver')
    # contextMultiprocess.set_forkserver_preload([path, savePath, mdbColAssets])


    with contextMultiprocess.Pool(processes=1) as pool:
        sequenceResults = pool.starmap(parseSequence, zip(sequences, itertools.repeat(path), itertools.repeat(savePath)))
